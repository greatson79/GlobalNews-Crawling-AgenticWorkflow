...
- NEVER DO 항목은 Anti-Rationalization 테이블로 강화: "흔한 합리화"와 "왜 안 되는가"를 명시하여 사전 인식 강화 (상세: `CLAUDE.md` Autopilot NEVER DO)
...
| 매 응답 후 | Phase-Aware Compact 제안 (`generate_context_summary.py`) | Phase 전환 또는 고사용 감지 시 /compact 제안 (비차단, 세션별 1회) |
...
| 세션 시작/복원 | Learned Patterns 자동 표면화 (`restore_context.py`) | 3+ 세션 반복 성공 패턴을 자동 표면화 (P1 Consumer) |
...
상세: `CLAUDE.md` ULW Mode 섹션 (Anti-Rationalization 테이블 포함), `AGENTS.md §5.1.1`
