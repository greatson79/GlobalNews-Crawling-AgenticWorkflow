...
│   │   ├── _context_lib.py                (공유 라이브러리 — 파싱, 생성, SOT 캡처, Smart Throttling, Autopilot 상태 읽기·검증, ULW 감지, 절삭 상수 중앙화, sot_paths() 경로 통합, 다단계 전환 감지, 결정 품질 태그 정렬, Error Taxonomy 12패턴+Resolution 매칭, Success Patterns(Edit/Write→Bash 성공 시퀀스 추출), IMMORTAL-aware 압축+감사 추적, E5 Guard 중앙화(is_rich_snapshot+update_latest_with_guard), Knowledge Archive 통합(archive_and_index_session — 부분 실패 격리), 경로 태그 추출(extract_path_tags), KI 스키마 검증(_validate_session_facts — RLM 필수 키 보장), SOT 스키마 검증(validate_sot_schema — 워크플로우 state.yaml 구조 무결성 9항목 검증: S1-S6 기본 + S7 pacs 구조(S7a-S7e: dimensions·score·weak_dimension·history·pre_mortem_flag) + S8 active_team 구조(S8a-S8e: name·status·tasks_completed·tasks_pending·completed_summaries) + S9 auto_approved_details 구조), Quality Gate 상태 추출(_extract_quality_gate_state — pacs/review/verification 로그에서 최신 단계 품질 게이트 상태를 IMMORTAL 스냅샷에 보존), Adversarial Review P1 검증(validate_review_output+parse_review_verdict+calculate_pacs_delta+validate_review_sequence — Enhanced L2 결정론적 검증), Translation P1 검증(validate_translation_output T1-T7 + check_glossary_freshness T8 + verify_pacs_arithmetic T9 범용 + validate_verification_log V1a-V1c), Predictive Debugging P1(aggregate_risk_scores+validate_risk_scores RS1-RS6+_RISK_WEIGHTS 13개 가중치+_RECENCY_DECAY_DAYS 감쇠), Cross-Step Traceability P1 검증(validate_cross_step_traceability CT1-CT5 + _TRACE_MARKER_RE + _HEADING_SLUG_RE), Domain Knowledge Structure P1 검증(validate_domain_knowledge DK1-DK7 + _DKS_REF_RE + _DKS_ID_RE), Abductive Diagnosis Layer(diagnose_failure_context 사전 증거 수집 + validate_diagnosis_log AD1-AD10 사후 검증 + _extract_diagnosis_patterns KA 아카이빙 + Fast-Path FP1-FP3 + 가설 우선순위 H1/H2/H3), Decision Log P1 검증(validate_decision_log DL1-DL8 + _DL_STEP_RE/_DL_CHECKPOINT_RE/_DL_DECISION_RE/_DL_RATIONALE_RE/_DL_TIMESTAMP_RE/_DL_EVIDENCE_RE 6개 regex + _DL_MIN_RATIONALE_WORDS=15), 모듈 레벨 regex 컴파일(9개+8개+8개+4개+6개+5개 패턴 — 프로세스당 1회), Importance-Based Retention(_importance_tier Tier 0-3 순수 함수 + validate_retention_result RI1-RI6 P1 검증 + cleanup_knowledge_index 3-step 중요도 기반 선택 + FIFO fail-safe), 반복 에러 타입 추출(extract_recurring_error_types — P1 Consumer, 전체 KI 에러 타입 세션별 집계), 학습된 패턴 추출(extract_learned_patterns — P1 Consumer, cross-session success_patterns 집계), Review Focus P1 검증(validate_review_focus FS1-FS3 — config/review-focus.yaml 기반 리뷰 집중 영역 소프트 검증), L0d Content Structure P1 검증(validate_output_structure L0d-heading/marker/count — config/output-structure.yaml 기반 산출물 구조 결정론적 검증), Autopilot Stall Detection(check_autopilot_progress + _AUTOPILOT_STALL_THRESHOLD=20 — 동일 단계 20 cycles 경고), Workflow Progress IMMORTAL(_extract_workflow_progress — SOT outputs+pACS 로그 기반 단계별 진행 현황 보존), Autopilot Decision History IMMORTAL(_extract_autopilot_decisions — autopilot-logs/step-*-decision.md 스캔 Rationale 첫 줄 보존), Team Merge 검증(validate_team_merge — SOT active_team.completed_summaries 키와 병합 산출물 교차 검증), KA 품질 아카이빙(_extract_verification_outcomes+_extract_review_outcomes+_extract_workflow_quality_summary — verification/review/pacs 로그에서 결과 추출 → KA 필드 enrichment), Retry Budget State IMMORTAL(_extract_retry_budget_state — gate별 재시도 카운트+최근 점수 추출 → 스냅샷 IMMORTAL 보존), S5b 완료 교차검증(validate_sot_schema S5b — workflow_status "completed" 시 current_step ≥ total_steps 검증))
...
├── src/                                  ← [자식] 핵심 소스 코드 (93개 모듈, ~42,200 LOC)
